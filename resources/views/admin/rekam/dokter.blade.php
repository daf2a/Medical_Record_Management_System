        ...
        <div class="p-4 sm:ml-64">
            <div class="py-12">
                ...
                @foreach($rekams as $rekam)
                    @if($rekam->name_dokter != $previousdokter)
                        @php
                        $previousdokter = $rekam->name_dokter;
                        @endphp

                        <h3 class="bg-blue-100 text-blue-800 text-2xl font-semibold px-2.5 y-6 py-1 mb-5 mt-8 rounded dark:bg-gray-700 dark:text-gray-200">{{ $rekam->name_dokter }}</h3>
                        <div class="container mx-auto gap-8 flex flex-col sm:flex-row flex-wrap">
                            @foreach($rekams as $rekamInner)
                                @if($rekamInner->name_dokter == $rekam->name_dokter)
                                    <div class="w-96 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <div class="p-5 justify-between">
                                            ...
                                            <div class="mb-3 grid grid-cols-1 gap-4">
                                                <div>
                                                    <p class="mb-1 font-normal text-sm text-gray-700 dark:text-white">Pasien : {{ $rekamInner->name_pasien }}</p>
